...
class Crud
{
  ...
  /*  
   * Método para recuperar o nome da tabela na propriedade $tabela  
   * @return $tabela
   */
  ...
  public function select($sql, $arrayParams = null, $fetchAll = TRUE)
  {
    if(strtoupper(substr($sql,0,6)) == "SELECT"){
      try {
        ...
        if (!empty($arrayParams)) :
          ...
          foreach ($arrayParams as $chave => $valor) :
            //verifica se a chave é uma string senão coloca o contador
            $chave = is_string($chave) ? $chave : $cont;
      ...
      return "Insira uma query de consulta";
    }
  }
}
